...
#[CoversClass(InvalidUnicodeRangeException::class)]
final class InvalidUnicodeRangeExceptionTest extends TestCase
{
    public function testItRetainsExceptionContext(): void
    {
        $previous = new InvalidArgumentException('root cause');
        $exception = new InvalidUnicodeRangeException('invalid range', 17, $previous);

        self::assertInstanceOf(FontExceptionInterface::class, $exception);
        self::assertSame('invalid range', $exception->getMessage());
